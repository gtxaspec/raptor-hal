...
{
	switch (fmt) {
	case RSS_PIXFMT_NV12:    return PIX_FMT_NV12;
	case RSS_PIXFMT_YUYV422: return PIX_FMT_YUYV422;
	default:                 return PIX_FMT_NV12;
	}
}
...
int hal_fs_set_rotation(void *ctx, int chn, int degrees)
{
...
#if defined(PLATFORM_T31)
	{
		...
		if (ret != 0) {
			HAL_LOG_ERR("GetChnAttr(%d) failed for rotation: %d",
				    chn, ret);
			return ret;
		}
		...
		case 270:
			...
			break;
		default:
			HAL_LOG_ERR("T31 rotation only supports 90/270, got %d",
				    degrees);
			return -EINVAL;
		}

		ret = IMP_FrameSource_SetChnRotate(chn, rot,
						   attr.picWidth,
						   attr.picHeight);
		if (ret != 0) {
			HAL_LOG_ERR("SetChnRotate(%d, %d) failed: %d",
				    chn, rot, ret);
			return ret;
		}
...
#elif defined(PLATFORM_T32) || defined(PLATFORM_T40) || defined(PLATFORM_T41)
	{
		...
		case 270:
			...
			break;
		default:
			HAL_LOG_ERR("I2D rotation only supports 90/270, got %d",
				    degrees);
			return -EINVAL;
		}
	...
	(void)chn;
	HAL_LOG_WARN("rotation not supported on this SoC (requested %d deg)",
		     degrees);
	return -ENOTSUP;
#endif
}
...
{
	switch (fmt) {
	case PIX_FMT_NV12:    return RSS_PIXFMT_NV12;
	case PIX_FMT_YUYV422: return RSS_PIXFMT_YUYV422;
	default:              return RSS_PIXFMT_NV12;
	}
}
...
int hal_fs_get_frame(void *ctx, int chn, void **frame_data,
...
int hal_fs_release_frame(void *ctx, int chn, void *frame_data)
{
	...
	if (ret != 0)
		HAL_LOG_ERR("IMP_FrameSource_ReleaseFrame(%d) failed: %d",
			    chn, ret);
...
int hal_fs_snap_frame(void *ctx, int chn, void **frame_data,
		      rss_frame_info_t *info)
{
	(void)ctx;
	int ret;
	...
	ret = IMP_FrameSource_SnapFrame(chn, PIX_FMT_NV12,
...
int hal_fs_set_max_delay(void *ctx, int chn, int max_delay_ms)
{
	...
	if (ret != 0)
		HAL_LOG_ERR("SetMaxDelay(%d, %d) failed: %d",
			    chn, max_delay_ms, ret);
	return ret;
#else
	(void)chn;
	(void)max_delay_ms;
...
int hal_fs_get_timed_frame(void *ctx, int chn, void *framets, int block,
                           void *framedata, void *frame)
{
	...
	return IMP_FrameSource_GetTimedFrame(chn,
