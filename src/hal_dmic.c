...
int hal_dmic_init(void *ctx, const rss_audio_config_t *cfg)
{
    ...
    attr.soundmode = (cfg->chn_count >= 2) ? DMIC_SOUND_MODE_STEREO : DMIC_SOUND_MODE_MONO;
    ...
    HAL_LOG_INFO("dmic init: rate=%d samples=%d depth=%d", cfg->sample_rate, cfg->samples_per_frame,
                 cfg->frame_depth);
    return RSS_OK;
...
int hal_dmic_enable_aec_ref_frame(void *ctx, int dev, int chn, int ao_dev, int ao_chn)
{
    (void)ctx;
    return IMP_DMIC_EnableAecRefFrame(dev, chn, ao_dev, ao_chn);
}

int hal_dmic_disable_aec_ref_frame(void *ctx, int dev, int chn, int ao_dev, int ao_chn)
{
    (void)ctx;
...
int hal_dmic_get_frame_and_ref(void *ctx, int dev, int chn, void *frame, void *ref, int block)
{
    (void)ctx;
    if (!frame || !ref)
        return RSS_ERR_INVAL;
    return IMP_DMIC_GetFrameAndRef(dev, chn, (IMPDmicChnFrame *)frame, (IMPDmicFrame *)ref,
                                   block ? BLOCK : NOBLOCK);
}
